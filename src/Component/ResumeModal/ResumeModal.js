...
import {
  resumeTemplates,
  chooseTemplates,
  imageresumeTemplates,
  modalValue,
  resumeTemplatesForFresher,
  authenticateduser,
} from "../../Recoil";
  ...
  const handleUserAuthenticationCheck = async () => {
    ...
    try {
      const response = await axios.post(
        `https://lizmyresumebuilder.onrender.com/user/auth/checkAuth`,
        {},
        { headers }
      );
      if (response) {
    ...
    <div>
      <div className={styles.preview_box} onClick={handleOpen}>
        ...
        <div className={styles.preview_template}>
          <img
            src={filterResume[0]?.resumeImg}
            alt={`template-${templateNo}`}
          />
        </div>
      </div>
      <Modal
            ...
            <div className={styles.template_box}>
              {imgtemplate.map((item, index) => (
                <div
                  ...
                  key={index}
                  onClick={() => handleFilterTemplates(item?.id)}
                >
                  <img
